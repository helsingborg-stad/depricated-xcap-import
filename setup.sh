...
read plugin_name
find ./ -type f ! -name "Init.sh" -exec sed -i '' -e "s/Kulturkortet Import/$plugin_name/g" {} >/dev/null 2>&1 \;

echo "\033[34m\033[1mEnter short plugin description:\033[0m "
read plugin_description
find ./ -type f ! -name "Init.sh" -exec sed -i '' -e "s/Imports events from Mittkulturkort.se/$plugin_description/g" {} >/dev/null 2>&1 \;
...
read plugin_slug
find ./ -type f ! -name "Init.sh" -exec sed -i '' -e "s/kulturkortet-import/$plugin_slug/g" {} >/dev/null 2>&1 \;

echo "\033[34m\033[1mEnter plugin name in capitals without punctiuation or white spaces LIKETHIS:\033[0m "
read plugin_cap
find ./ -type f ! -name "Init.sh" -exec sed -i '' -e "s/KULTURKORTETIMPORT/$plugin_cap/g" {} >/dev/null 2>&1 \;

echo "\033[34m\033[1mEnter plugin namespace:\033[0m "
read plugin_namespace
find ./ -type f ! -name "Init.sh" -exec sed -i '' -e "s/KulturkortetImport/$plugin_namespace/g" {} >/dev/null 2>&1 \;

echo "\033[34m\033[1mEnter plugin author name:\033[0m "
read plugin_author
find ./ -type f ! -name "Init.sh" -exec sed -i '' -e "s/Kristoffer Svanmark, Sebastian Thulin/$plugin_author/g" {} >/dev/null 2>&1 \;
